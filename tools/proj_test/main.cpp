...
int main(int /*argc*/, char *argv[])
{
    ...
    /*  for (size_t f = 0; f < final_image.width() * final_image.height(); f++)
      {
          final_image.channel(0)[f] = 0;
          final_image.channel(1)[f] = 0;
          final_image.channel(2)[f] = 0;
          final_image.channel(3)[f] = 65535;
      } */
    ...
    /*
        for (auto gcp : operation_t.ground_control_points)
        {
            auto projfunc = [&projector_final](float lat, float lon, int, int) -> std::pair<int, int>
            {
                int x, y;
                projector_final.forward(lon, lat, x, y);
                return {x, y};
            ...
            final_image.draw_circle(pos.first, pos.second, 2, color, true);
        }
        */

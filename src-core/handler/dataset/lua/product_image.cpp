...
namespace satdump
{
    namespace lua
    {
        void bind_product_image(sol::state &lua)
        {
            ...
            lua["generate_expression_product_composite"] = sol::overload(
                // (image::Image(*)(products::ImageProduct *, std::string))(&products::generate_expression_product_composite)
                [](products::Product *p, std::string c)
                {
                    ...
                    return products::generate_expression_product_composite((products::ImageProduct *)p, c);
                });

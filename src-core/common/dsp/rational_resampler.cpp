...
namespace dsp
{
    ...
    FFRationalResamplerBlock::FFRationalResamplerBlock(std::shared_ptr<dsp::stream<float>> input, unsigned interpolation, unsigned decimation, std::vector<float> custom_taps)
        : Block(input),
          d_interpolation(interpolation),
          d_decimation(decimation),
          d_ctr(0)
    ...
    FFRationalResamplerBlock::~FFRationalResamplerBlock()
    {
        for (int i = 0; i < nfilt; i++)
            volk_free(taps[i]);
        volk_free(taps);
    ...
    void FFRationalResamplerBlock::work()
    {
        int nsamples = input_stream->read();
        if (nsamples <= 0)
        {
        ...
        memcpy(&buffer[ntaps], input_stream->readBuf, nsamples * sizeof(float));
        in_buffer = ntaps + nsamples;
        input_stream->flush();
        ...
        for (int i = 0; i < in_buffer - ntaps;)
        {
            volk_32f_x2_dot_prod_32f(&output_stream->writeBuf[outc++], &buffer[i], taps[d_ctr], ntaps);
            d_ctr += this->d_decimation;
            while (d_ctr >= d_interpolation)
            {
                d_ctr -= d_interpolation;
        ...
        memmove(&buffer[0], &buffer[in_buffer - ntaps], ntaps * sizeof(float));

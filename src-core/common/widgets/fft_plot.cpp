...
namespace satdump
{
    namespace widgets
    {
        ...
        void FFTPlot::draw(ImVec2 size)
        {
            ...
            const ImRect inner_bb({frame_bb.Min.x, frame_bb.Min.y + style.FramePadding.y}, {frame_bb.Max.x - 1, frame_bb.Max.y - style.FramePadding.y});
            ...
            if (enable_freq_scale && bandwidth != 0 && frequency != 0)
            {
                ...
                for (int i = 1; i < 10; i++)
                {
                    ...
                    window->DrawList->AddText({x_line - ImGui::CalcTextSize(cstr.c_str()).x / 2, y_level - 30 * ui_scale}, i == 5 ? (ImU32)style::theme.green : col_base, cstr.c_str());
                    if (i == 5 && actual_sdr_freq != -1)
                    {
                        auto cstr = "(" + format_notated(actual_sdr_freq) + ")";
                        window->DrawList->AddText({x_line - ImGui::CalcTextSize(cstr.c_str()).x / 2, y_level - 16 * ui_scale}, i == 5 ? (ImU32)style::theme.red : col_base, cstr.c_str());
                    }
                }
            }
            ...
            if (bandwidth != 0 && frequency != 0 && vfo_freqs.size() > 0)
            {
                ...
                for (auto vfo : vfo_freqs)
                {
                    ...
                    if (sqrtf(powf(ImGui::GetMousePos().x - x_line, 2) + powf(ImGui::GetMousePos().y - y_level, 2)) < 40 * ui_scale)
                        window->DrawList->AddText({x_line - ImGui::CalcTextSize(vfo.first.c_str()).x / 2, y_level - 45 * ui_scale}, col_base_vfo, vfo.first.c_str());
                }
            }

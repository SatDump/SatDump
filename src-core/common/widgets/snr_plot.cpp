...
namespace widgets
{
    void SNRPlotViewer::draw(float snr, float peak_snr)
    {
        ImGui::Text("SNR (dB) : ");
        ImGui::SameLine();
        ImGui::TextColored(snr > 2 ? snr > 10 ? IMCOLOR_GREEN : IMCOLOR_ORANGE : IMCOLOR_RED, "%0.3f", snr);

        ImGui::Text("Peak SNR (dB) : ");
        ImGui::SameLine();
        ImGui::TextColored(peak_snr > 2 ? peak_snr > 10 ? IMCOLOR_GREEN : IMCOLOR_ORANGE : IMCOLOR_RED, "%0.3f", peak_snr);
        ...
        ImGui::SameLine();
        ImGui::TextColored(avg_snr > 2 ? avg_snr > 10 ? IMCOLOR_GREEN : IMCOLOR_ORANGE : IMCOLOR_RED, "%0.3f", avg_snr);

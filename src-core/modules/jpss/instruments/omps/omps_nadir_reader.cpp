...
namespace jpss
{
    namespace omps
    {
        ...
        void OMPSNadirReader::work(ccsds::CCSDSPacket &packet)
        {
            if (packet.header.sequence_flag == 1)
            {
                if (currentOMPSFrame.size() > 1000)
                {
                    if (packet.header.apid == 616) // Compressed, JPSS-1 and over
                    {
        ...
        cimg_library::CImg<unsigned short> OMPSNadirReader::getChannel(int channel)
        {
            return cimg_library::CImg<unsigned short>(channels[channel], 142, lines);
        }
    } // namespace atms
} // namespace jpss
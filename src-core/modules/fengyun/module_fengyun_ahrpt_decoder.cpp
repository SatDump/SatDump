...
namespace fengyun
{
    FengyunAHRPTDecoderModule::FengyunAHRPTDecoderModule(std::string input_file, std::string output_file_hint, std::map<std::string, std::string> parameters) : ProcessingModule(input_file, output_file_hint, parameters),
                                                                                                                                                                ...
                                                                                                                                                                viterbi1(true, d_viterbi_ber_threasold, 1, d_viterbi_outsync_after, 50),
                                                                                                                                                                viterbi2(true, d_viterbi_ber_threasold, 1, d_viterbi_outsync_after, 50)
    {
    ...
    void FengyunAHRPTDecoderModule::process()
    {
        ...
        while (!data_in.eof())
        {
            ...
            if (time(NULL) % 10 == 0 && lastTime != time(NULL))
            {
                ...
                logger->info("Progress " + std::to_string(round(((float)data_in.tellg() / (float)filesize) * 1000.0f) / 10.0f) + "%, Viterbi 1 : " + viterbi1_state + " BER : " + std::to_string(viterbi1.ber()) + ", Viterbi 2 : " + viterbi2_state + " BER : " + std::to_string(viterbi2.ber()) + ", Deframer : " + deframer_state);
            }
        }
    ...
    {
        ImGui::Begin("FengYun AHRPT Decoder", NULL);

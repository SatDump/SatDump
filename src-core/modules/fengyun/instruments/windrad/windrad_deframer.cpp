...
#define ASM_SYNC1 0xfaf355aa
#define ASM_SYNC2 0xfaf3aabb
...
namespace fengyun
{
    namespace windrad
    {
        WindRADDeframer1::WindRADDeframer1()
        {
            // Default values
            writeFrame = false;
            wroteBits = 0;
        ...
        void WindRADDeframer1::pushBit(uint8_t bit)
        {
        ...
        std::vector<std::vector<uint8_t>> WindRADDeframer1::work(uint8_t *input, int size)
        {
            // Output buffer
            ...
            for (int byteInBuf = 0; byteInBuf < size; byteInBuf++)
            {
                ...
                for (int i = 7; i >= 0; i--)
                {
                    ...
                    shifter = ((shifter << 1 | bit) % 4294967296); // 2^32
                    ...
                    if (writeFrame)
                    {
                        ...
                        if (shifter == ASM_SYNC1)
                        {
                            // Fill up what we're missing
                            for (int b = 0; b < 13160 - outputBits; b++)
                                pushBit(0);
        ...
        WindRADDeframer2::WindRADDeframer2()
        {
            // Default values
            writeFrame = false;
            wroteBits = 0;
        ...
        // Write a single bit into the frame
        void WindRADDeframer2::pushBit(uint8_t bit)
        {
            byteBuffer = (byteBuffer << 1) | bit;
            wroteBits++;
            if (wroteBits == 8)
        ...
        std::vector<std::vector<uint8_t>> WindRADDeframer2::work(uint8_t *input, int size)
        {
            // Output buffer
            std::vector<std::vector<uint8_t>> framesOut;
            ...
            for (int byteInBuf = 0; byteInBuf < size; byteInBuf++)
            {
                ...
                for (int i = 7; i >= 0; i--)
                {
                    ...
                    if (writeFrame)
                    {
                        ...
                        // New ASM, ABORT! and process the new one
                        if (shifter == ASM_SYNC2)
                        {
                            // Fill up what we're missing
                            for (int b = 0; b < 18072 - outputBits; b++)
                                pushBit(0);

...
namespace fengyun_svissr
{
    ...
    void SVISSRDecoderModule::drawUI(bool window)
    {
        ...
        ImGui::BeginGroup();
        {
            // Constellation
            {
                ...
                {
                    draw_list->AddCircleFilled(ImVec2(ImGui::GetCursorScreenPos().x + (int)(100 + (buffer[i] / 127.0) * 130) % 200,
                                                      ImGui::GetCursorScreenPos().y + (int)(100 + rng.gasdev() * 14) % 200),
                                               2,
    ...
    std::shared_ptr<ProcessingModule> SVISSRDecoderModule::getInstance(std::string input_file, std::string output_file_hint, std::map<std::string, std::string> parameters)
    {

...
namespace angels
{
    namespace argos
    {
        ...
        void AngelsArgosDecoderModule::process()
        {
            ...
            while (!data_in.eof())
            {
                ...
                if (vcdu.vcid == 1)
                {
                    ...
                    {
                        if (pkt.header.apid == 1047)
                        {
                            argos_ccsds++;
                            if (pkt.payload.size() >= 32)
                            {
                                frames_out.write((char *)pkt.header.raw, 6);
                                frames_out.write((char *)pkt.payload.data(), 32);
                            }
                        }
                    }
                }
        ...
        std::shared_ptr<ProcessingModule> AngelsArgosDecoderModule::getInstance(std::string input_file, std::string output_file_hint, std::map<std::string, std::string> parameters)
        {

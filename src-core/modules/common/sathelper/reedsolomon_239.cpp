/*
 * reedsolomon.cpp
 *
 *  Created on: 04/11/2016
 *      Author: Lucas teske
 */

#include "reedsolomon_239.h"
#include <cstring>

namespace sathelper
{

    unsigned char ToDualBasis239[256] = {};

    unsigned char FromDualBasis239[256] = {};

    ReedSolomon239::ReedSolomon239()
    {
        this->rs = correct_reed_solomon_create(correct_rs_primitive_polynomial_ccsds, 120, 11, 16);
    }

    ReedSolomon239::~ReedSolomon239()
    {
        correct_reed_solomon_destroy(this->rs);
    }

    uint32_t ReedSolomon239::decode_ccsds(uint8_t *data)
    {
        for (int i = 0; i < 255; i++)
        {
            data[i] = FromDualBasis239[data[i]];
        }

        int errors = decode_rs8(data);

        for (int i = 0; i < 255; i++)
        {
            data[i] = ToDualBasis239[data[i]];
        }

        return errors;
    }

    uint32_t ReedSolomon239::decode_rs8(uint8_t *data)
    {
        uint8_t odata[255];
        int err = correct_reed_solomon_decode(this->rs, data, 255, odata);
        if (err == -1)
        {
            return -1;
        }
        else
        {
            err = 0;
            // Calculate wrong bytes
            for (int i = 0; i < 239; i++)
            {
                if ((data[i] ^ odata[i]) != 0)
                {
                    err++;
                }
            }
        }
        std::memcpy(data, odata, 255);
        return err;
    }

    void ReedSolomon239::deinterleave(uint8_t *data, uint8_t *output, uint8_t pos, uint8_t I)
    {
        for (int i = 0; i < 255; i++)
        {
            output[i] = data[i * I + pos];
        }
    }

    void ReedSolomon239::interleave(uint8_t *data, uint8_t *output, uint8_t pos, uint8_t I)
    {
        for (int i = 0; i < 255; i++)
        {
            output[i * I + pos] = data[i];
        }
    }
}
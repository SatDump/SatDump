...
namespace satdump
{
    namespace ndsp
    {
        ...
        DecimatingFIRBlock<T, TT>::DecimatingFIRBlock(std::string id)
            : BlockSimple<T, T>(id + "decimating_fir" + (std::is_same_v<TT, complex_t> ? "c" : "") + "_" + getShortTypeName<T>() + getShortTypeName<T>(), {{"in", getTypeSampleType<T>()}},
                                {{"out", getTypeSampleType<T>()}})
        {
        }

        template <typename T, typename TT>
        DecimatingFIRBlock<T, TT>::~DecimatingFIRBlock()
        {
            if (taps != nullptr)
            {
        ...
        template <typename T, typename TT>
        void DecimatingFIRBlock<T, TT>::init()
        {
            // Free if needed
            if (taps != nullptr)
            {
        ...
        template <typename T, typename TT>
        uint32_t DecimatingFIRBlock<T, TT>::process(T *input, uint32_t nsamples, T *output)
        {
            if (needs_reinit)
            {
                needs_reinit = false;
        ...
        template class DecimatingFIRBlock<complex_t, complex_t>;
        template class DecimatingFIRBlock<complex_t, float>;
        template class DecimatingFIRBlock<float, float>;
    } // namespace ndsp
} // namespace satdump
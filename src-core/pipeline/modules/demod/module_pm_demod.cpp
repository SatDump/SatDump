    ...
    namespace pipeline
    {
        namespace demod
        {
            ...
            void PMDemodModule::init()
            {
                ...
                // Domain conversion
                pm_psk = std::make_shared<dsp::PMToBPSK>(pll->output_stream, d_resample_after_pll ? d_samplerate : final_samplerate, d_subccarier_offset == 0 ? d_symbolrate : d_subccarier_offset);
                ...
                // RRC
                rrc = std::make_shared<dsp::FIRBlock<complex_t>>(d_resample_after_pll ? agc2->output_stream : pm_psk->output_stream,
                                                                 dsp::firdes::root_raised_cosine(1, final_samplerate, d_symbolrate, d_rrc_alpha, d_rrc_taps));
            ...
            PMDemodModule::~PMDemodModule() { delete[] sym_buffer; }
            ...
            nlohmann::json PMDemodModule::getModuleStats()
            {
            ...
            std::string PMDemodModule::getID() { return "pm_demod"; }

...
namespace satdump
{
    namespace image
    {
        ...
        Image::Image(int bit_depth, size_t width, size_t height, int channels) { init(bit_depth, width, height, channels); }
                ...
                for (int x = 0; x < new_width; x++)
                    for (int y = 0; y < new_height; y++)
                        memcpy((uint8_t *)new_data + ((new_width * new_height * c) + y * new_width + x) * type_size,
                               (uint8_t *)d_data + (c * d_width * d_height + (y0 + y) * d_width + (x + x0)) * type_size, type_size);
        ...
        void Image::crop(int x0, int x1) { crop(x0, 0, x1, d_height); }
        ...
        Image Image::crop_to(int x0, int x1) { return crop_to(x0, 0, x1, d_height); }
            ...
            for (int cc = 0; cc < d_channels; cc++)
            {
                for (int i = 0; i < height; i++)
                {
                    for (int j = 0; j < width; j++)
                    {
                        ...
                        val = a * (1 - x_diff) * (1 - y_diff) + b * (x_diff) * (1 - y_diff) + c * (y_diff) * (1 - x_diff) + d * (x_diff * y_diff);
        ...
        int Image::get_pixel_bilinear(int cc, double rx, double ry)
        {
            ...
            a = (float)a * a_a;
            int ret = clamp(a * (1 - x_diff) * (1 - y_diff) + b * (x_diff) * (1 - y_diff) + c * (y_diff) * (1 - x_diff) + d * (x_diff * y_diff));
            if (d_channels == 4 && cc != 3)
            {
                ret = (float)ret / (a_a * (1 - x_diff) * (1 - y_diff) + b_a * (x_diff) * (1 - y_diff) + c_a * (y_diff) * (1 - x_diff) + d_a * (x_diff * y_diff));
            }
        ...
        void imemcpy(Image &img1, size_t pos1, Image &img2, size_t pos2, size_t px_size)
        {
            ...
            memcpy((uint8_t *)img1.raw_data() + pos1 * img1.typesize(), (uint8_t *)img2.raw_data() + pos2 * img1.typesize(), px_size * img1.typesize());
        }

...
int main(int argc, char *argv[])
{
    ...
    while (current_ptr < msg_file.size())
    {
        GenericRecordHeader main_header(&msg_file[current_ptr]);
        ...
        {
            std::string full_header(&msg_file[current_ptr + 20], &msg_file[current_ptr + 20] + main_header.record_size - 20);
            printf("%s\n", full_header.c_str());
        }

        /*if (main_header.record_class == 2)
        {
            uint64_t value_idk = msg_file[current_ptr + 69 + 0] << 32 |
                                 msg_file[current_ptr + 69 + 1] << 24 |
                                 msg_file[current_ptr + 69 + 2] << 16 |
                                 msg_file[current_ptr + 69 + 3] << 8 |
                                 msg_file[current_ptr + 69 + 4];
        ...
        if (main_header.record_class == 8)
        {
            uint16_t number_of_samples =
                msg_file[current_ptr + 22 + 0] << 8 |
                msg_file[current_ptr + 22 + 1];
            ...
            for (int c = 0; c < 5; c++)
            {
                ...
                {
                    int16_t val;
                    ((uint8_t *)&val)[1] = msg_file[current_ptr + 24 + (c * number_of_samples + i) * 2 + 0];
                    ((uint8_t *)&val)[0] = msg_file[current_ptr + 24 + (c * number_of_samples + i) * 2 + 1];
        ...
        {
            double lat_first = get_deg_latlon(&msg_file[current_ptr + 20538 + 0]);
            double lon_first = get_deg_latlon(&msg_file[current_ptr + 20538 + 4]);

            double lat_last = get_deg_latlon(&msg_file[current_ptr + 20546 + 0]);
            double lon_last = get_deg_latlon(&msg_file[current_ptr + 20546 + 4]);
            ...
            for (int i = 0; i < 103; i += 2)
            {
                ...
                if (px >= 2048)
                    continue;
                double lat = get_deg_latlon(&msg_file[current_ptr + 21380 + i * 8 + 0]);
                double lon = get_deg_latlon(&msg_file[current_ptr + 21380 + i * 8 + 4]);
            ...
            logger->info("%d %d %d %f %f %f %f", (int)main_header.record_class, (int)main_header.record_size, (int)main_header.record_subclass_version,
                         lon_first, lat_first, lon_last, lat_last);
        }

...
namespace satdump
{
    namespace xrit
    {
        ...
        inline image::Image getImageFromXRITFile(xrit_file_type_t type, XRITFile &file)
        {
            ...
            if (type == XRIT_FY4_AGRI)
            {
                logger->trace("Using FY-4x Specific Image Header!");
                fy4::ImageInformationRecord image_structure_record = file.getHeader<fy4::ImageInformationRecord>(); //(&lrit_data[all_headers[ImageStructureRecord::TYPE]]);

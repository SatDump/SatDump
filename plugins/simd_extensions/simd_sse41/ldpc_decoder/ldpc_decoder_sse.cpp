...
namespace codings
{
    namespace ldpc
    {
        LDPCDecoderSSE::LDPCDecoderSSE(Sparse_matrix pcm) : LDPCDecoder(pcm)
        {
            int max_deg = 0;
            ...
            d_vns = new __m128i[d_pcm_num_vn];
            d_vns_to_cn_msgs = new __m128i[d_pcm_max_cn_degree];
            d_cn_to_vn_msgs = new __m128i[d_pcm_num_cn * d_pcm_max_cn_degree];
            d_abs_msgs = new __m128i[d_pcm_max_cn_degree];

            d_vn_addr = new __m128i *[d_pcm_num_edges];
        ...
        void LDPCDecoderSSE::generic_cn_kernel(int cn_idx)
        {
            ...
            if (cn_deg & 0x1)
                parity = _mm_xor_si128(_mm_set1_epi16(0xFFFF), parity);
            ...
            for (int vn_idx = 0; vn_idx < cn_deg; vn_idx++)
            {
                ...
                abs_msg = _mm_abs_epi16(msg);

                /* Determine the first and second minimum */
                min2 = _mm_min_epi16(_mm_max_epi16(min1, abs_msg), min2);
                min1 = _mm_min_epi16(abs_msg, min1);

...
namespace satdump
{
    ...
    {
        void MHSNatProcessor::ingestFile(std::vector<uint8_t> nat_file)
        {
            int image_width = 90;
...

            {
                ...
                mhs_products = std::make_shared<satdump::products::ImageProduct>();
                mhs_products->instrument_name = "mhs";

                mhs_products->set_product_source(info.sat_name);
                mhs_products->set_product_timestamp(ptime);

                auto p = loadJsonFile(resources::getResourcePath("projections_settings/metop_abc_mhs.json"));
#if 0
            mhs_products.set_proj_cfg_tle_timestamps(p, info.satellite_tle, timestamps);
#else
        ...
        std::shared_ptr<satdump::products::Product> MHSNatProcessor::getProduct() { return mhs_products; }
    } // namespace firstparty
} // namespace satdump
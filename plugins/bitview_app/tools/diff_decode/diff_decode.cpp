...
namespace satdump
{
    std::string DifferentialTool::getName() { return "Differential Decoder"; }

    void DifferentialTool::renderMenu(std::shared_ptr<BitContainer> &container, bool is_busy)
    {
        if (is_busy)
            style::beginDisabled();
    ...
    bool DifferentialTool::needToProcess() { return should_process; }

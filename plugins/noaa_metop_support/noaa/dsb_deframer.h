...
namespace noaa
{
    ...
    */
    ...
    {
    public:
        const uint16_t FRM_ASM = 0xEDE2;             // ASM
        const uint16_t FRM_ASM_INV = FRM_ASM ^ 0xFFFF; // Inverted ASM
        const int FRM_ASM_SIZE = 16;                 // ASM bit length
        ...
        int STATE_SYNCED = 7;  // SYNCED state. We found a lock and confirmed it, skipping bit-level search and with high tolerance.
    ...
    public:
        DSB_Deframer(int frm_size, int padding);
        ~DSB_Deframer();

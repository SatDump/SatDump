        ...
        image::Image<uint16_t> IASIIMGReader::getIRChannel()
        {
            ...
            for (size_t y2 = 0; y2 < img.height() / 64; y2++)
            {
                ...
                for (int y = 0; y < 64; y++)
                {
                    for (int x = 0; x < 64; x++)
                    {
                        count_cold[x][y] = ((double)img[(y2 * 64 + y) * 36 * 64 + (0 * 64) + x] + (double)img[(y2 * 64 + y) * 36 * 64 + (1 * 64) + x]) / 2.0;
                        count_warm[x][y] = ((double)img[(y2 * 64 + y) * 36 * 64 + (3 * 64) + x] + (double)img[(y2 * 64 + y) * 36 * 64 + (4 * 64) + x]) / 2.0;
                    }
                }
                ...
                for (int x2 = 0; x2 < 36; x2++)
                {
                    for (int y = 0; y < 64; y++)
                    {
                        for (int x = 0; x < 64; x++)
                        {
                            ...
                            if (initial < 0)
                                initial = 0;
                            img[(y2 * 64 + y) * 36 * 64 + (x2 * 64) + x] = initial;
                        }
                    }
                }

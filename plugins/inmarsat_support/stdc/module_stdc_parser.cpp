...
namespace inmarsat
{
    namespace stdc
    {
        ...
        void STDCParserModule::drawUI(bool window)
        {
            ...
            ImGui::SameLine();
            ImGui::TextColored(IMCOLOR_GREEN, "%d", last_pkt_count);
                ...
                if (ImGui::BeginTabItem("Messages"))
                {
                    ...
                    for (int i = pkt_history_msg.size() - 1; i >= 0; i--)
                    {
                        ...
                        try
                        {
                            ...
                            ImGui::TextColored(IMCOLOR_LAVENDER, "%s", get_id_name(id).c_str());
                    ...
                    for (int i = pkt_history.size() - 1; i >= 0; i--)
                    {
                        ...
                        try
                        {
                            ...
                            else if (id == pkts::PacketEGCDoubleHeader1::FRM_ID || id == pkts::PacketEGCDoubleHeader2::FRM_ID)
                            {
                                ...
                                ImGui::TextColored(IMCOLOR_YELLOW, "%s", timestampToTod(msg["timestamp"].get<double>()).c_str());
                                ImGui::TableSetColumnIndex(2);
                                ImGui::TextColored(IMCOLOR_RED, "%s", msg["message"].get<std::string>().c_str());
                            }
                            else
                            {
                                ImGui::TableNextRow();

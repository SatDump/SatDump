...
struct Block0Header
{
    ...
    uint8_t CZONE;     // Current compensation zone (0–32) is an 8-bit integer number. A zero value indicates that no compensation is being performed. Values 1–32 denote the latitudinal zone for which
    ...
    int16_t PBIAS;    // E-W grid bias (0 ± 12546 pixels) is a signed 15-bit integer number denoting the pixel offset employed for the grid data. A value of zero indicates the grid is not shifted from
                      // the locations computed using the current O&A.
    ...
    float GPATH;      // Most recent range calibration ground path delay: a floating point value denoting the number of 50-MHz clock counts that the GVAR signal takes to transit through CDA station
                      // equipment.
    float XMSNE;      // The call tower range calibration value – a floating point value denoting the number of 50-MHz clock counts that the GVAR signal takes to transit the satellite transmission
                      // electronics.
